...
def _generate_sitemap_files():
    ...
    for pkg in search_results:
        ...
        _add_url_to_sitemap(file_root, pkg_url, datetime.fromisoformat(pkg["metadata_modified"]),
        ...
        if INC_RESOURCES:
            for res in resources:
                ...
                _add_url_to_sitemap(file_root, res_url, datetime.fromisoformat(res["created"]), tk.config.get(
    ...
    if INC_GROUPS:
        groups = (
            ...
            .filter(sa.or_(model.Group.type == "organization", model.Group.type == "group"))
            .all()
        )
